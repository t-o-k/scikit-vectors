...
```shell
pip3 install --user scikit-vectors
```

or

```shell
python3 -m pip install --user scikit-vectors
```
...
...     )
>>> SP3.basis_α(), SP3.basis_β(), SP3.basis_γ()
(SP3(α=1, β=0, γ=0), SP3(α=0, β=1, γ=0), SP3(α=0, β=0, γ=1))
...
>>> u = SP3(x, y, z)
>>> u.normalize()
SP3(α=x/sqrt(x**2 + y**2 + z**2), β=y/sqrt(x**2 + y**2 + z**2), γ=z/sqrt(x**2 + y**2 + z**2))
>>> v = SP3(a, b, c)
>>> w = SP3(d, e, f)
>>> v.are_parallel(w)
...
>>> w = SP3(d, e, f)
>>> v.project(w)
SP3(α=d*(a*d + b*e + c*f)/(d**2 + e**2 + f**2), β=e*(a*d + b*e + c*f)/(d**2 + e**2 + f**2), γ=f*(a*d + b*e + c*f)/(d**2 + e**2 + f**2))
>>> v = SP3(a, b, c)
>>> w = SP3(d, e, f)
>>> v.sin(w)
...
>>> u = SP3(x, y, z)
>>> u.rotate_α(angle=e)
SP3(α=x, β=y*cos(e) - z*sin(e), γ=y*sin(e) + z*cos(e))
>>> u = SP3(x, y, z)
>>> u(lambda s: a + s * b)
SP3(α=a + b*x, β=a + b*y, γ=a + b*z)
```
